...
| 파일 | 책임 |
|---|---|
| `types.py` | A2A 메시지/태스크 타입 (`Message`, `Part`, `Role`, `TaskState`) — 메시징 / Task / FSM 가이드는 [messaging.md](./messaging.md) 참조 |
...
flowchart LR
    ...
    Cfg --> B["build_agent_card()<br/>공통 함수 (shared 패키지)"]
    B --> C[["AgentCard 인스턴스<br/>(메모리 상주)"]]
...
이 흐름은 두 가지 관심사로 분리된다:
**(1) 배포 토폴로지** — 하나의 이미지에서 여러 컨테이너가 생성되지만 Config 가 다르다.
**(2) 부팅 시퀀스** — 각 컨테이너 내부에서 Config 가 AgentCard 인스턴스로 변환된다.

### 4.1. 배포 관점 — 이미지 1개, Config 는 컨테이너마다

```mermaid
flowchart LR
    Img[("📦 Docker Image<br/>shared + runtime")]
    Img --> Eng["engineer 컨테이너<br/>+ engineer config"]
    Img --> Arch["architect 컨테이너<br/>+ architect config"]
    Eng -->|exposes| EngCard[["AgentCard A<br/>(engineer:backend)"]]
    Arch -->|exposes| ArchCard[["AgentCard B<br/>(architect)"]]
```

빌드 산출물(이미지) 에는 AgentCard 값이 박히지 않는다.
Config 마운트 차이가 각 컨테이너를 서로 다른 에이전트로 만든다.

### 4.2. 컨테이너 내부 부팅 흐름 — 모든 에이전트 공통

```mermaid
flowchart LR
    Cfg[/"config/base.yaml<br/>config/override.yaml"/]
    Cfg --> L["load_config()"]
    L --> B["build_agent_card()"]
    B --> C[["AgentCard 인스턴스"]]
    C --> R(["GET /.well-known/agent-card.json"])
```

부팅 시 **단 한 번** 실행된다. 결과 인스턴스는 `app.state` 등에 저장해
매 요청마다 재사용한다 (Config 재파싱 없음).

### 4.3. 핵심 3가지

1. **이미지는 하나, 인스턴스는 N개** — 빌드 산출물에 역할 정보 없음.
2. **한 번만 만들어 재사용** — 부팅 시 빌드, 이후 요청마다 같은 인스턴스를 직렬화.
3. **Config 가 다르면 카드가 다르다** — 마운트만 바꾸면 완전히 다른 에이전트.
