...
| 항목 | 우선 | 상태 | 구현 |
|---|---|---|---|
| **Upstream 전체 스트림 timeout** (U1) | 🟥 Must | ✅ 반영됨 | `main.py::chat` 을 `anyio.fail_after(UG_UPSTREAM_TOTAL_TIMEOUT_S)` 로 감쌈. 기본 300s, 초과 시 `{type:"error",message:"upstream timeout after Ns"}` |
| **httpx.Limits 튜닝** (U2) | 🟨 Should | ✅ 반영됨 | `lifespan` 에서 `httpx.Limits(max_connections, max_keepalive_connections)` 명시. `UG_UPSTREAM_MAX_CONN` / `UG_UPSTREAM_MAX_KEEPALIVE` 환경변수 override |
| **Upstream retry / backoff** (U3) | 🟨 Should | ✅ 반영됨 | 초기 connect error / 5xx 시 지수 backoff 로 `UG_UPSTREAM_CONNECT_RETRIES` 회 재시도 (기본 2). 스트리밍 중간 실패는 재시도 불가 (토큰 이미 방출) |
...
| 항목 | 우선 | 상태 | 구현 |
|---|---|---|---|
| **CORS 정책** (U4) | 🟨 Should | ✅ 반영됨 | `CORSMiddleware` + `UG_ALLOWED_ORIGINS` 환경변수 (콤마 구분 allowlist). 빈 값이면 같은 origin 만 |
...
| **A2A → FE 번역 테이블 공식화** (U6) | 🟨 Should | ✅ 반영됨 | 본 문서 §6 매핑 표. 코드 구현은 `main.py::chat` 의 kind-분기 |
...
| 항목 | 우선 | 상태 | 구현 |
|---|---|---|---|
| **Cache-Control 헤더** (U8) | 🟨 Should | ✅ 반영됨 | `CacheControlMiddleware`: `/assets/*` → `public, max-age=31536000, immutable` (Vite hash 안전), `/` 및 `*.html` → `no-cache` |
