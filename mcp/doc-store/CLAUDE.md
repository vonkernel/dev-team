...
## 2. Collection 10종 — 절대 추가 / 변경은 별도 이슈

#75 재설계로 chat tier (UG↔P/A) + A2A tier (에이전트 간) + 도메인 산출물 분리.

### Chat tier

| Collection | 정체성 |
|---|---|
| `sessions` | UG↔P/A 의 한 대화창 |
| `chats` | session 안의 한 발화 (immutable) |
...
**원칙**:
- 새 collection 추가 = **별도 이슈 + 사용자 컨펌** 후. 즉흥 추가 금지.
- 기존 collection 의 컬럼 변경 = **마이그레이션 신규 파일 작성** (기존 SQL 수정 X).
- PRD 는 별도 entity 안 만든다. `wiki_pages.page_type='prd'` 로 통합.
...
```
1. mcp/doc-store/migrations/NNN_<name>.sql        새 마이그레이션 (forward + .rollback.sql)
2. src/doc_store_mcp/schemas/<name>.py            Pydantic 모델 (Create / Update / Read)
3. src/doc_store_mcp/repositories/<name>.py       AbstractRepository 상속 + 특수 쿼리
4. src/doc_store_mcp/tools/<name>.py              MCP tool 함수 (6 op + 특수 도구) — Pydantic 파라미터 직접 사용 (mcp/CLAUDE.md §1.3.1)
5. src/doc_store_mcp/tools/__init__.py            import 1줄 추가
```
...
## 4. 노출 도구 면 — collection 별 6 op

각 collection 이 동일한 6 op 를 노출 (mcp/CLAUDE.md §1.5 표준). 도구 파라미터 / 반환은 **Pydantic 모델 직접** (mcp/CLAUDE.md §1.3.1).

| 도구 | 시그니처 | 비고 |
|---|---|---|
| `{collection}.create` | `(doc: <Entity>Create) → <Entity>Read` | 신규 |
| `{collection}.update` | `(id, patch: <Entity>Update, expected_version?) → <Entity>Read \| None` | id 미존재 시 None. version 있는 entity 는 optimistic locking |
| `{collection}.get` | `(id) → <Entity>Read \| None` | 단건 |
| `{collection}.list` | `(where?, limit, offset, order_by) → list[<Entity>Read]` | filter 는 단순 equality |
| `{collection}.delete` | `(id) → bool` | hard delete |
| `{collection}.count` | `(where?) → int` | 페이지네이션용 |

immutable collections (`chats`, `a2a_messages`, `a2a_task_status_updates`,
`a2a_task_artifacts`) 는 `update` 미노출 (5 op + special).

특수 도구 (read 패턴 전용):
- `chats.list_by_session(session_id) → ordered list`
- `assignments.list_by_session(root_session_id) → list`
- `a2a_contexts.find_by_context_id(context_id) → A2AContext | null`
- `a2a_messages.list_by_context(a2a_context_id) → ordered list`
- `a2a_messages.list_by_task(a2a_task_id) → ordered list` (Task.history)
- `a2a_tasks.find_by_task_id(task_id) → A2ATask | null`
- `a2a_task_status_updates.list_by_task(a2a_task_id) → ordered list`
- `a2a_task_artifacts.list_by_task(a2a_task_id) → ordered list`
...
- 스키마 본문: [`docs/doc-store-schema.md`](../../docs/doc-store-schema.md)
