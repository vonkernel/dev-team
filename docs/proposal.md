...
graph TD
    ...
    subgraph EventPipe["대화 기록 파이프라인"]
        Broker["Redis Streams<br/>(Message Broker)"]
...
graph TD
    subgraph Container["Docker Container (1 Agent)"]
        ...
        BrokerClient["Redis 클라이언트<br/>(대화 이벤트 publish 전용)"]
    end

    Peers["다른 에이전트들"]
    Librarian["Librarian<br/>(질의/Diff 수신 대상)"]
    Broker["Redis Streams<br/>(대화 로그 브로커)"]
    ...
    %% BrokerClient는 RoleConfig와 무관 — 환경변수로 Redis 설정 주입
...
> **대화 로그 수집은 Librarian의 역할이 아니다.** 별도의 경량 Consumer인 Chronicler가 Redis Streams 브로커를 통해 수집·저장한다. (섹션 2.6 참조)
...
| **대화 로그 기록** | 에이전트 → Redis Streams → Chronicler → MCP → Document DB | A2A 대화 이력 수집 (Librarian 경유하지 않음) |
...
### 2.6. A2A 대화 이벤트 수집 (Redis Streams + Chronicler)

에이전트 간 직접 소통은 A2A(요청-응답)로 이루어지지만, **대화 로그는 Redis Streams 브로커로 publish**되어 **Chronicler**라는 경량 Consumer가 Document DB에 영속화한다. 이 분리는 다음을 보장한다:
...
graph LR
    ...
    Stream["Redis Streams<br/>XADD → a2a-events"]
...
| 구현 | 단일 Python 스크립트 수준 — Redis 클라이언트 + Doc DB MCP 클라이언트만 보유 |
...
- 에이전트는 Redis 클라이언트로 `XADD a2a-events * <field> <value> ...` 호출
...
- 발신 에이전트가 `session_id`, `item_id`, `prev_item_id`를 부여하여 Redis Streams에 XADD
...
| **Message Broker** | **Redis Streams** | **A2A 대화 이벤트 publish용 (단순 구성 유지, 추상화 불필요)** |
| **Chronicler** | **경량 Python Consumer (에이전트 아님)** | **Redis Streams 구독 → Document DB 영속화** |
...
> **에이전트가 아닌 보조 모듈:**
> **Chronicler** — Redis Streams를 구독하여 A2A 대화 이벤트를 Document DB에 영속화하는 경량 Consumer. LLM, LangGraph, Role Config를 사용하지 않는 단순 Python 스크립트 수준의 모듈. 에이전트 역할 정의에서 다루지 않으며, 인프라로 취급. (섹션 2.6 참조)
...
services:
  ...
  primary:
    ...
    depends_on: [doc-db-mcp, external-pm-mcp, redis]
  ...
  # --- 대화 기록 파이프라인 (Redis Streams + Chronicler) ---
  redis:
...
│       ├── broker/                # Redis Streams publish 헬퍼
...
│   ├── Dockerfile
│   └── src/
│       └── main.py                # Redis Streams 구독 → Doc DB MCP 저장
...
| 14 | 이벤트 수집 파이프라인 | Redis Streams 브로커 + **Chronicler** 경량 Consumer가 Document DB에 영속화 (Librarian 책임 아님) |
...
5. **대화 기록 파이프라인 PoC** — 에이전트 → Redis Streams → Chronicler → Document DB 흐름, Task/Session/Item 계층 저장 및 조회 동작 검증
