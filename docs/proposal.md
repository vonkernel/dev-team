...
- 이중 계층 공유 메모리(Atlas + Doc Store)를 **Librarian**이 관리하여 팀의 메모리/컨텍스트로 활용
...
| 역할 | 입력 | 처리 | 출력 |
|------|------|------|------|
| **Diff 색인** | Eng가 전달한 diff (파일 목록 + 파일 내 변경) | 변경 사항을 OO 구조로 해석 | Atlas 노드/관계 업데이트 |
| **지식 기록** | 에이전트가 전달한 기술 개념/아이디어/주의사항/TODO | 분류하여 문서화 | Doc Store에 태스크 연관 문서 저장 |
| **조회 응답** | 에이전트의 자연어 질의 | Atlas + Doc Store 교차 쿼리 | 정리된 답변 반환 |
...
| **Task Context 조립** | Eng/QA → A2A → `get_task_context(task_id)` → Atlas | Code Agent 호출 전 필요한 파일/참조 시그니처 집합 조회 |
| **대화 로그 기록** | 에이전트 → Valkey Streams → Chronicler → MCP → Doc Store | A2A 대화 이력 수집 (Librarian 경유하지 않음) |
...
sequenceDiagram
    ...
    participant GraphMCP as Atlas MCP
...
| Doc Store | 추상화 인터페이스 (기본: **PostgreSQL + JSONB**) | 기록/대화/문서 (Episodic Layer), 추후 교체 가능. ※ Postgres 를 선택한 맥락은 §6.4 참조 |
...
7. A가 수행하는 후처리:
    ...
    - **미채택안**: Librarian을 통해 Doc Store에 alternatives 태그로 저장
8. A → L: 채택 설계의 OO 구조를 Atlas에 색인 요청
