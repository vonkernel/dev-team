...
**역할:**
- 사용자 채팅 입력을 A2A `SendMessage` 또는 `SendStreamingMessage` 로 변환하여 Primary 또는 Architect 에게 전달
- **SSE streaming** 활용 — 에이전트 응답(LLM 토큰, 중간 상태)을 실시간 UI 로 렌더링
- Primary/Architect 가 사용자에게 전달할 메시지를 수신하여 UI 로 렌더링
...
**라우팅 규칙:**
- 기획 관련 대화 (요구사항, PRD, 일정 등) → Primary로 전달
- 기술 관련 대화 (설계, 기술 선택, 구현 조율) → Architect로 전달
- 사용자가 명시적으로 대상을 지정하면 그쪽으로 전달
- 사용자의 초기 요청은 기본적으로 Primary로 전달
