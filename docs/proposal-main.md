...
- **사용자는 P 와 A 양쪽과 직접 소통** 하여 프로젝트 기획 / 기술 설계 양방향으로 개입 가능
...
| 2단계 | OO 설계 | P, A, 사용자, (필요 시 L) | A의 서브 에이전트 루프, 사용자 기술 개입 수용, OO 1차 설계 확정 |
| 3단계 | 병렬 구현·검증 | A, Eng+QA 페어들, (필요 시 L) | Eng 자체 루프 + QA 독립 테스트. 산출물 직접 write. 자기 변경 코드는 Eng 자체 색인 (Atlas 직접 write) |
...
6. **Eng 자체 Atlas 색인 PoC** — Eng 이 자기 변경 diff 분석 → Atlas MCP 직접 write 로 OO 구조 색인 (분담 모델 옵션 A 확정 — `#63`)
...
9. **Context Assembly + OpenCode 제어 PoC**
    - Eng / QA 가 자기 도메인 데이터를 Atlas MCP 직접 read (정보 검색 / 교차 쿼리는 L 자연어 위임)
    - 프롬프트 조립기 구현
