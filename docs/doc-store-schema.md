...
**역할**: 에이전트가 처리하는 한 단위 일. Chronicler 의 Task 계층 (architecture-event-pipeline) 그대로. 모든 다른 entity (issue / wiki / session) 의 묶음 키.
...
**역할**: 한 agent_task 안에서 진행되는 한 대화 흐름 (architecture-event-pipeline 의 Session). A2A `contextId` 와 1:1.
...
**역할**: session 안의 한 메시지 (architecture-event-pipeline 의 Item). prev_item_id 로 시간순 chain.
