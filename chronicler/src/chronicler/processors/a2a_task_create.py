...
publisher-supplied id 패턴: event.context_id / task_id 모두 UUID.
task_id (= a2a_tasks.id) dedup.
"""
...
class A2ATaskCreateProcessor(EventProcessor):
    ...
    async def process(self, event: A2AEvent, db: DocStoreClient) -> None:
        ...
        existing = await db.a2a_task_get(event.task_id)
        if existing is not None:
            logger.debug(
                "a2a.task.create skip — task_id=%s 이미 존재", event.task_id,
            )
            return
        ...
        if ctx is None:
            logger.warning(
                "a2a.task.create skip — context_id=%s 미존재",
                event.context_id,
            )
            return
        ...
        logger.info(
            "a2a.task.create task_id=%s state=%s", event.task_id, event.state,
        )
